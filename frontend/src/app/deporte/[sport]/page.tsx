...
export default async function Page({
  ...
  params: { sport: string }
  searchParams?: { comp?: string; tab?: string }
}) {
  ...
  if (!comps.length) {
    return (
      <div className={styles.page}>
        <p>Deporte sin competencias configuradas.</p>
      </div>
    )
  }
  ...
  const hasData = fixtures.length > 0 || standings.length > 0

  return (
    <div className={styles.page}>
      <h1 className={styles.sportTitle}>
        {sport === 'futbol' ? 'Fútbol Argentino' : 'Básquet Argentino'}
      </h1>
      ...
      <div className={styles.compTabs}>
        {comps.map((c) => (
          <Link
            key={c.slug}
            href={`/deporte/${sport}?comp=${c.slug}`}
            className={activeComp === c.slug ? styles.compTabActive : styles.compTab}
          >
            {c.label}
          </Link>
        ))}
      ...
      <div className={styles.compTabs}>
        <Link
          href={`/deporte/${sport}?comp=${activeComp}&tab=fixture`}
          className={activeTab === 'fixture' ? styles.compTabActive : styles.compTab}
        >
          Fixture
        </Link>
        <Link
          href={`/deporte/${sport}?comp=${activeComp}&tab=tabla`}
          className={activeTab === 'tabla' ? styles.compTabActive : styles.compTab}
        >
          Tabla
        </Link>
        <Link
          href={`/deporte/${sport}?comp=${activeComp}&tab=resultados`}
          className={activeTab === 'resultados' ? styles.compTabActive : styles.compTab}
        >
          Resultados
        </Link>
      </div>
      ...
      <p className={styles.sportSubtitle}>
        Fuente: {data.source_used || 'Sin datos'} · Actualizado:{' '}
      ...
      <div className={styles.layout}>
        <section className={styles.section}>
          {activeTab !== 'tabla' &&
            (activeTab === 'resultados' ? results : fixtures).map((f) => (
              <div key={f.id} className={styles.matchTableRow}>
                <span>
                  {f.datetime_arg
                    ? new Date(f.datetime_arg).toLocaleString('es-AR')
                    : 'Fecha sin confirmar'}
                </span>
                <span>
