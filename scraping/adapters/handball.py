"""Handball — TheSportsDB (IHF) + Sofascore."""
import logging, re
...
def _make(raw, src):
    ...
    return NormalizedMatch(id=f"{SPORT}-{src}-{hn}-{an}", sport=SPORT, source=src,
        competition=comp or "Handball", home_team=h, away_team=a,
    ...
    async def scrape(self):
        ...
        try:
            for fn in [get_events_today, get_next_events]:
                for ev in await fn(4841):  # IHF Handball
                    raw = parse_tsdb(ev, SPORT); add(_make(raw, "tsdb"))
            logger.info(f"[handball/tsdb] {len(matches)}")
        except Exception as e: logger.warning(f"[handball/tsdb] {e}")
        if not matches:
            try:
            ...
            except Exception as e: logger.warning(f"[handball/sofascore] {e}")
        logger.info(f"[handball] TOTAL {len(matches)}"); return matches
