...
def _make(raw, src):
    ...
    return NormalizedMatch(id=f"{SPORT}-{src}-{hn}-{an}", sport=SPORT, source=src,
        competition=comp or "Boxeo", home_team=h, away_team=a,
    ...
    async def scrape(self):
        ...
        try:
            for fn in [get_events_today, get_next_events]:
                for ev in await fn(4443):  # Boxing
                    raw = parse_tsdb(ev, SPORT); add(_make(raw, "tsdb"))
            logger.info(f"[boxeo/tsdb] {len(matches)}")
        except Exception as e: logger.warning(f"[boxeo/tsdb] {e}")
        if not matches:
            try:
            ...
            except Exception as e: logger.warning(f"[boxeo/sofascore] {e}")
        logger.info(f"[boxeo] TOTAL {len(matches)}"); return matches
