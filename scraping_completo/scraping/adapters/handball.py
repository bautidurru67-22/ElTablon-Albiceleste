"""Handball argentino — TheSportsDB (IHF) + Sofascore fallback."""
import logging, re
...
def _make(raw: dict, src: str) -> NormalizedMatch | None:
    ...
    return NormalizedMatch(
        ...
        competition=comp or "Handball IHF",
...
class HandballAdapter(BaseScraper):
    async def scrape(self) -> list[NormalizedMatch]:
        ...
        # TheSportsDB — IHF Handball
        try:
            for ev in await get_events_today(4841):
                raw = parse_event(ev, SPORT)
                raw["competition"] = raw.get("competition") or "IHF Handball"
                add(_make(raw, "tsdb"))
            logger.info(f"[handball/tsdb] {len(matches)}")
        except Exception as e:
            logger.warning(f"[handball/tsdb] {e}")
        # Sofascore fallback
        if not matches:
            try:
                for fn in [ss_today, ss_live]:
                    ...
                    for m in sofascore_normalizer.normalize_events(data.get("events", []), SPORT):
                        if m.id not in seen:
                            seen.add(m.id); matches.append(m)
                logger.info(f"[handball/sofascore] {len(matches)}")
            except Exception as e:
                logger.warning(f"[handball/sofascore] {e}")
