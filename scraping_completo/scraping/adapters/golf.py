"""Golf argentino — TheSportsDB (PGA/European Tour) + Sofascore fallback."""
import logging, re
...
class GolfAdapter(BaseScraper):
    async def scrape(self) -> list[NormalizedMatch]:
        ...
        try:
            for lid in [4378, 4379]:  # PGA Tour, European Tour
                for ev in await get_events_today(lid):
                    raw = parse_event(ev, SPORT)
                    add(_make(raw, "tsdb"))
            logger.info(f"[golf/tsdb] {len(matches)}")
        except Exception as e:
            logger.warning(f"[golf/tsdb] {e}")
        if not matches:
            try:
                for fn in [ss_today, ss_live]:
                    ...
                    for m in sofascore_normalizer.normalize_events(data.get("events", []), SPORT):
                        if m.id not in seen:
                            seen.add(m.id); matches.append(m)
                logger.info(f"[golf/sofascore] {len(matches)}")
            except Exception as e:
                logger.warning(f"[golf/sofascore] {e}")
